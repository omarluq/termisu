...
describe Termisu::Events::Key do
  describe ".new" do
    it "creates a key event with default modifiers" do
      event = Termisu::Events::Key.new(Termisu::Key::LowerA)
    ...
    it "creates a key event with specified modifiers" do
      event = Termisu::Events::Key.new(Termisu::Key::LowerA, Termisu::Modifier::Ctrl)
      event.key.should eq(Termisu::Key::LowerA)
    ...
    it "creates a key event with combined modifiers" do
      ...
      event = Termisu::Events::Key.new(Termisu::Key::LowerA, mods)
      event.modifiers.ctrl?.should be_true
  ...
  describe "#ctrl?" do
    ...
    it "returns false when Ctrl modifier is not set" do
      event = Termisu::Events::Key.new(Termisu::Key::LowerA, Termisu::Modifier::Shift)
  ...
  describe "#alt?" do
    it "returns true when Alt modifier is set" do
      event = Termisu::Events::Key.new(Termisu::Key::LowerA, Termisu::Modifier::Alt)
  ...
  describe "#meta?" do
    it "returns true when Meta modifier is set" do
      event = Termisu::Events::Key.new(Termisu::Key::LowerA, Termisu::Modifier::Meta)
  ...
  describe "#ctrl_c?" do
    it "returns true for Ctrl+C" do
      event = Termisu::Events::Key.new(Termisu::Key::LowerC, Termisu::Modifier::Ctrl)
    ...
    it "returns false for Ctrl+UpperC (implementation checks lowercase only)" do
      ...
      event = Termisu::Events::Key.new(Termisu::Key::UpperC, Termisu::Modifier::Ctrl)
      event.ctrl_c?.should be_false
    end

    it "returns false for just 'c'" do
      event = Termisu::Events::Key.new(Termisu::Key::LowerC)
      event.ctrl_c?.should be_false
    end
  ...
  describe "#ctrl_d?" do
    it "returns true for Ctrl+D" do
      event = Termisu::Events::Key.new(Termisu::Key::LowerD, Termisu::Modifier::Ctrl)
      event.ctrl_d?.should be_true
    end

    it "returns false for just 'd'" do
      event = Termisu::Events::Key.new(Termisu::Key::LowerD)
  ...
  describe "#ctrl_z?" do
    it "returns true for Ctrl+Z" do
      event = Termisu::Events::Key.new(Termisu::Key::LowerZ, Termisu::Modifier::Ctrl)
      event.ctrl_z?.should be_true
    end

    it "returns false for just 'z'" do
      event = Termisu::Events::Key.new(Termisu::Key::LowerZ)
  ...
  describe "#ctrl_q?" do
    it "returns true for Ctrl+Q" do
      event = Termisu::Events::Key.new(Termisu::Key::LowerQ, Termisu::Modifier::Ctrl)
      event.ctrl_q?.should be_true
    end

    it "returns false for just 'q'" do
      event = Termisu::Events::Key.new(Termisu::Key::LowerQ)
  ...
  describe "#char" do
    ...
    it "returns nil for non-printable keys" do
      event = Termisu::Events::Key.new(Termisu::Key::Up)
      event.char.should be_nil
    end

    it "returns nil for function keys" do
      event = Termisu::Events::Key.new(Termisu::Key::F1)
      event.char.should be_nil
    end

    it "returns space character for Space key" do
      event = Termisu::Events::Key.new(Termisu::Key::Space)
  ...
  describe "special keys" do
    it "handles Escape key" do
      event = Termisu::Events::Key.new(Termisu::Key::Escape)
      event.key.escape?.should be_true
    end

    it "handles Enter key" do
      event = Termisu::Events::Key.new(Termisu::Key::Enter)
      event.key.enter?.should be_true
      event.char.should eq('\n')
    end

    it "handles Backspace key" do
      event = Termisu::Events::Key.new(Termisu::Key::Backspace)
      event.key.backspace?.should be_true
    end

    it "handles Tab key" do
      event = Termisu::Events::Key.new(Termisu::Key::Tab)
      event.key.tab?.should be_true
      event.char.should eq('\t')
    end

    it "handles BackTab (Shift+Tab)" do
      event = Termisu::Events::Key.new(Termisu::Key::BackTab)
  ...
  describe "arrow keys" do
    ...
    it "handles Down arrow" do
      event = Termisu::Events::Key.new(Termisu::Key::Down)
      event.key.down?.should be_true
    end

    it "handles Left arrow" do
      event = Termisu::Events::Key.new(Termisu::Key::Left)
      event.key.left?.should be_true
    end

    it "handles Right arrow" do
      event = Termisu::Events::Key.new(Termisu::Key::Right)
      event.key.right?.should be_true
    end

    it "handles arrow keys with Shift modifier" do
      event = Termisu::Events::Key.new(Termisu::Key::Up, Termisu::Modifier::Shift)
      event.key.up?.should be_true
      event.shift?.should be_true
    end
  end
  ...
  describe "function keys" do
    ...
    it "handles F12" do
      event = Termisu::Events::Key.new(Termisu::Key::F12)
      event.key.f12?.should be_true
    end

    it "handles function keys with modifiers" do
      mods = Termisu::Modifier::Ctrl | Termisu::Modifier::Shift
      event = Termisu::Events::Key.new(Termisu::Key::F5, mods)
