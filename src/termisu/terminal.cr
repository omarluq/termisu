# Unified terminal interface combining TTY I/O and Termios state management.
#
# Provides a high-level API for terminal operations, managing both
# the underlying TTY file descriptors and terminal attributes (raw mode).
#
# Example:
# ```
...
class Termisu::Terminal
  ...
  # Creates a new terminal instance, opening `/dev/tty` for I/O.
  #
  ...
  # Closes the terminal, disabling raw mode and closing TTY.
  def close
    disable_raw_mode
    @tty.close
  end
